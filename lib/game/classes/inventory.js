...
Inventory = ig.Class.extend({
	...
	itemUses: {
		...
		rock: {
			use: function(player) {
				ig.game.inventory.removeFromInventory(ig.game.inventory.searchInventory('rock'));
	...
	trySell: function(item) {
		if(item.sellable === false || item.price == undefined) return;

		ig.game.state.money += Math.round(item.price / 2);
	...
	itemClicked: function(item) {
		...
		else if(item.type == 'equippable') this.equipItem(item);
		else if(item.type == 'consumable') this.useConsumableItem(item);
	},
