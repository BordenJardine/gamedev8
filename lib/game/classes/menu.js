...
Menu = ig.Class.extend({
	...
	toggle: function() {
		...
		if(this.menuImageOpen) {
			...
			if(this.canSell) ig.game.inventoryHUD.open = true;
		}
		else {
			ig.game.bindKeys();
			...
			if(this.canSell) ig.game.inventoryHUD.open = false;
		}
	},
	...
	searchForItem: function(itemName){
		var realName = itemName.substr(this.name.length);

		for(var i = 0; i < this.items.length; i++) {
			if(this.items[i].name == realName) {
				return this.items[i];
			}
		}
	},
	...
	addMenuGUIButton: function(item) {
		...
		ig.gui.element.add({
			...
			title: (item.price != undefined) ? '$'+item.price.toFixed(2).toString() : '1',
			showTitle: true,
