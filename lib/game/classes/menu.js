...
Menu = ig.Class.extend({
	...
	init: function(items) {
		...
		this.menuImage.pos = {
			...
			y: ig.system.width / 2 - this.menuImage.image.height / 2
		};
		this.setupItems();
	},

	toggle: function() {
		this.menuImage.open = !this.menuImage.open;
		if(this.menuImage.open) ig.game.unbindKeys();
		else {
			ig.game.bindKeys();
	...
	arrangeGUI: function() {
		...
		for(var col = 0; col < columns; col+=1) {
			for(var row = 0; row < 5; row++) {
				...
				if(!item) break;
				item.pos = {
					x: this.menuImage.pos.x + 6 + (col*32),
					y: this.menuImage.pos.y + 34 + (row*32)};
