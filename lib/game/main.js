...
ClosingScene = Scene.extend({
	...
	draw: function() {
		...
		ig.system.context.fillText( 'THE END', this.center.x, this.center.y + 40 - this.step);

		ig.system.context.font = 'bold 48px sans-serif';
		ig.system.context.fillText("Created By", this.center.x, 1024 - this.step);
		ig.system.context.fillText("Jusion and MSPain", this.center.x, 1124 - this.step);

		ig.system.context.font = 'bold 32px sans-serif';
		ig.system.context.fillText("With art by The White Dragon", this.center.x, 1224 - this.step);
		ig.system.context.fillText("and assistance from HardDisk", this.center.x, 1324 - this.step);

		//if (step > 1324) drawImage();
...
homeless = Scene.extend({
	...
	drawCountdown: function() {
		...
		if (this.sec === 0) this.getEntitiesByType(EntityPlayer)[0].die();
