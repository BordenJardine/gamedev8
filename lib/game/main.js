...
homeless = Scene.extend({
	...
	drawGUI: function() {
		...
		if(this.inventory.open) {
			...
			var invenCount = this.inventoryGUI.length - 1;
			if (invenCount == 0) invenCount = 1;
			var columns = Math.ceil(invenCount/5);

			for(var col = 0; col < columns; col+=5) {
				for(var inv = col; inv < (col-1)+5; inv++) {
					if(!this.inventoryGUI[inv] || inv > columns) break;
					var item = this.inventoryGUI[inv];
					item.pos = {x: this.inventory.pos.x + 6 + (32*col), y: this.inventory.pos.y  + 34 + (inv*32)};
