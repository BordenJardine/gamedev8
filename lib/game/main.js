...
homeless = Scene.extend({
	...
	init: function() {
		...
		if(this.loadGame()) {
			...
			else this.levelDirector.jumpTo(this.state.currentLevel);
		...
		if(this.loaded) {
			...
			if(this.state.inventory.length > 0) {
				for (var i = 0; i < this.state.inventory.length; i++) {
					addInventoryGUIButton(this.state.inventory[i]);
				}
	...
	drawObjectives: function() {
		...
		if(length > 0) {
			if(this.lastObjectivesLength === 0) {
				this.font.draw('Current Objectives:'.slice(0,this.objectivesDrawCounter), this.hud.pos.x, this.hud.pos.y+30, ig.Font.ALIGN.LEFT);
			} else {
			...
			if(this.lastObjectivesLength != length) {
				for(var i = 0; i < length; i++) {
					if(i === length - 1) {
						this.font.draw(this.state.objectives[length-1].slice(0, this.objectivesDrawCounter), this.hud.pos.x, this.hud.pos.y+30+(14*(i+1)), ig.Font.ALIGN.LEFT);
					}
					else this.font.draw(this.state.objectives[i], this.hud.pos.x, this.hud.pos.y+30+(14*(i+1)), ig.Font.ALIGN.LEFT);
				}
			...
			if(this.objectivesDrawCounter === this.newObjective.length) {
