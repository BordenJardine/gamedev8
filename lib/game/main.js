		...
		if(length > 0) {
			...
			if(this.lastObjectivesLength != length) {
				for(var i = 0; i < length; i++) {
					if(i === length - 1) {
						this.fontsmall.draw(this.state.objectives[length-1].slice(0, this.objectivesDrawCounter), this.hud.pos.x, this.hud.pos.y+30+(14*(i+1)), ig.Font.ALIGN.LEFT);
					}
					else this.fontsmall.draw(this.state.objectives[i], this.hud.pos.x, this.hud.pos.y+30+(14*(i+1)), ig.Font.ALIGN.LEFT);
				}
				this.objectivesDrawCounter++;
			} else {
				for(var i = 0; i < length; i++) {
					this.fontsmall.draw(this.state.objectives[i], this.hud.pos.x, this.hud.pos.y+30+(14*(i+1)), ig.Font.ALIGN.LEFT);
				}
			}
