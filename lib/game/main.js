...
ig.global.ITEM_USES = { //starting to think this jazz belongs in item class
	box: {
		use: function(player) {
			if(!player.inBox) {
				player.inBox = true;
				...
				for(var i = 0; i < inven.length; i++) {
					...
					if(item.name == this.name) {
						...
						if(item.type == 'consumable') {
							ig.global.ITEM_USES[this.name].use(ig.game.getEntitiesByType(EntityPlayer)[0]);
							if(item.quantity > 1) item.quantity--;
							else ig.game.state.inventory.splice(i, 1);
						}
					}
				}
		...
		if(this.inventory.open) {
			...
			for(var col = 0; col < columns; col+=1) {
				for(var row = 0; row < 5; row++) {
					...
					if(!item) break;
					item.pos = {x: this.inventory.pos.x + 6 + (32*col), y: this.inventory.pos.y + 34 + (row*32)};
