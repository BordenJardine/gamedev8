    ...
    ig.global.ITEM_USES = {
        box: {
            use: function(player) {
                if(!player.inBox) {
                    player.inBox = true;
                    player.maxVel = {x:ig.game.state.equipped.itemStrength,y:ig.game.state.equipped.itemStrength};
                    player.currentAnim = player.anims.boxHide;
                } else {
                    player.inBox = false;
                    player.maxVel = {x: 100, y: 100};
    ...
    */
    ...
    homeless = Scene.extend({
    	...
    	init: function() {
    		...
    		*/
    		this.levelDirector = new ig.Director(this, [LevelTestlevel, LevelAI, LevelUnderworld1, LevelTutorial, LevelUberwelt]);
        ...
        drawGUI: function() {
            ...
            if(this.inventory.open) {
                ...
                for(var inv = 0; inv < this.state.inventoryGUI.length; inv++) {
                    ...
                    item.pos = {x: this.inventory.pos.x + 12, y: this.inventory.pos.y + 42 + (inv*32)};
