...
EntityLaser = ig.Entity.extend({
	...
	ready: function() {
		...
		if (this.standalone === 'true') {
			...
			this.standaloneTimer.set(Math.floor(Math.random() * 5) + 12);
		}
	...
	fire: function() {
		if (!this.down) {
			this.firing = true;
			if (!this.standalone) this.preFire.set(this.owner.health / 2);
			else this.preFire.set(5);
		}
	},
	...
	knockDebris: function() {
		...
		if (debrisOut < 10) {
			...
			for (var i = 0; i < numPieces; i++) {
				...
				ig.game.spawnEntity(EntityItem, x, y, { spawnedItem: true, image: 8, type: 'equippable', name: 'debris' });
			}
		}
	},
	...
	update: function() {
		if (this.firing) {
			...
			if (this.preFire.delta() >= 0) {
				// SOOOOO UGLY CLEAN ME UP PLEASEEEE
				if(!this.standalone) ig.game.theScreenShaker.timedShake(50, .5);
				if (!this.standalone) ig.game.spawnEntity(EntityLaserblast, this.pos.x, this.pos.y, { lifetime: this.owner.health/2, owner: this });
				else ig.game.spawnEntity(EntityLaserblast, this.pos.x, this.pos.y, { lifetime: 5, owner: this, size: 'small' });
