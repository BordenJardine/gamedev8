...
EntityLaser = ig.Entity.extend({
	...
	fire: function() {
		if (!this.down) {
			this.firing = true;
			if (!this.standalone) this.preFire.set(this.owner.health);
	...
	update: function() {
		if (this.firing) {
			...
			if (this.preFire.delta() >= 0) {
				...
				if (!this.standalone) ig.game.spawnEntity(EntityLaserblast, this.pos.x, this.pos.y, { lifetime: this.owner.health, owner: this });
