...
EntityVendor = ig.Entity.extend({
	...
	ready: function() {
		if(this.type === "metrocard") this.items = [{name:'metrocard', price: 2.25, quantity: 10, image: 5}];
		if(this.type === "metrocard") {
			this.animSheet = new ig.AnimationSheet('media/metro.png', 32, 32);
			this.addAnim('idle', 1, [0]);
			this.currentAnim = this.anims.idle;
		}
