		...
		switch(this.dialogChangeTrigger) {
			case 'item':
				...
				if(item) {
					...
					if(item.removeOnCompletion === true || item.removeOnCompletion === 'true') ig.game.inventory.removeFromInventory(item);
					if(nextDialog.reward != undefined) {
            if (nextDialog.reward === 'item') {
              ig.game.inventory.addToInventory(nextDialog.rewardItem)
            }
            else ig.game.state.money += nextDialog.reward;
          }
					this.dialogIdx++;
			...
			case 'initialItem':
				...
				if(item) {
					...
					ig.game.inventory.removeFromInventory(item);
					this.dialogChangeTrigger = undefined;
				}
				break;
			case 'complete':
