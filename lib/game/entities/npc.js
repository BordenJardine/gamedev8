...
EntityNpc = EntitySmart.extend({
	...
	ready: function() {
        		...
        		if(this.triggeredDialogKey) {
        			this.eventChain = this.eventChainGenerator.getChain(this.triggeredDialogKey, this.finished, this);
	...
	check: function(other) {
		if(other && other instanceof EntityPlayer && !(this instanceof EntityBaddy)) {
			if(ig.input.state("action") && !this.dialogActive) {
				if(this.dialogKey) this.eventChain = this.eventChainGenerator.getChain(this.dialogKey, this.finished, this);
				this.dialogActive = true;
				ig.game.talker = this.talkImage;
			}
		}
