		...
		if(other && other instanceof EntityPlayer) {
			if (ig.input.pressed('action') && ig.game.state.flags.indexOf('cart') > -1) {
				if(this.item != undefined) {
					var itemTest = ig.gui.element.action('getByName', this.item.name);
					if(itemTest == undefined) {
						ig.game.state.inventory.push(this.item);
						ig.game.addInventoryGUIButton(this.item);
					} else {
						if(itemTest.quantity != undefined) itemTest.quantity += 1;
						else itemTest.quantity = 1;
					}
					ig.game.newPopup('You scavenged a ' + this.item.name);
					this.item = undefined;
				}
			}
		}
